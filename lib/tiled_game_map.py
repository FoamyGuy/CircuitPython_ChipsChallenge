...
class TiledGameMap(Group):
    ...
    def __init__(
        ...
        self.player_loc = {"x": 4, "y": 3}
        self.prev_player_loc = self.player_loc.copy()
        ...
        if hasattr(self, 'player_entity_class'):
        ...
        if hasattr(self, 'npc_entity_map'):
            entity_map = self.npc_entity_map
        else:
            entity_map = None
        if hasattr(self, 'player_property'):
    ...
    def _load_tilegrids(self, x=0, y=0, width=10, height=8, player_property="player",
        ...
        print(f"tile props: {self.tile_properties}")
        ...
        for index, tile_index in enumerate(_map_tiles):
            ...
            if tile_index != 0:
                ...
                if load_entities_from_map:
                    ...
                    if _player_layer_index != 0:
                        print(f"{_player_layer_index - 1}, {player_property}")
                        print(self.get_tile_property(_player_layer_index - 1, player_property))
                    ...
                    #print(f"entity types: {npc_entity_map}")
                    for entity_type in npc_entity_map.keys():
                        #print(f"{_player_layer_index - 1}, {entity_type}")
                        #print(self.get_tile_property(_player_layer_index - 1, "name"))
                        if self.get_tile_property(_player_layer_index - 1, "name") == entity_type:
                            _entity_map_loc = {'x': (_x + x), 'y': (_y + y)}
                            # create a entity obj
        ...
        for _entity in self.entities:
            if _entity.tilegrid not in self:
                self.append(_entity.tilegrid)
    ...
    def get_layer_property(self, layer_index, prop_name):
        #print(self.map_obj['layers'][layer_index])
        for prop in self.map_obj['layers'][layer_index]['properties']:
